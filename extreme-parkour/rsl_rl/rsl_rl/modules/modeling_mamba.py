...
'''
dim: 模型的隐藏维度（即 token 的嵌入维度）。

mixer_cls: 用于处理 token 序列的主模块（例如 MyMamba

norm_cls: 标准化方法LayerNorm 或 RMSNorm默认 LayerNorm。

fused_add_norm: 是否融合 Add + Norm 操作以优化性能（使用 Triton/Fused 内核）。

residual_in_fp32: 是否将残差分支转换为 float32 精度，提高数值稳定性
'''
class MyMambaBlock(nn.Module):
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            # 使用 Triton fused 层：hidden_states + residual -> norm
...
class MyMambaModel(MyMambaPreTrainedModel):
    def __init__(
        ...
        self.gru = nn.GRU(input_size=config.d_model, hidden_size=256, batch_first=True)
        self.final_proj = nn.Linear(256, 7)
    ...
    def forward(
        self,
        inputs_embeds: torch.FloatTensor,  # 只保留这个输入
    ...
    # 直接处理输入的 embeddings
        hidden_states = inputs_embeds # (B, T, D)
        ...
        # last_setps = gru_out[:, -1, :]  # 只取最后一个时间步
        # 时间歩平均
        pooled = torch.mean(gru_out, dim=1)
        # # 注意力池化
        # attn_weights = self.attn_pool(gru_out)  # (B, T, 1)
        # attn_weights = torch.softmax(attn_weights, dim=1)  # (B, T, 1)
        # pooled = torch.sum(gru_out * attn_weights, dim=1)  # (B, 256)
